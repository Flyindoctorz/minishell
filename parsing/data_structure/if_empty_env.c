...
/*   Updated: 2025/04/14 17:14:06 by cgelgon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    char **envp = ft_calloc(4, sizeof(char *));
    if (!envp)
        return (NULL);
...
{
    t_env *pwd = ft_envnew(ft_strdup("PWD"), ft_strdup(data->cwd));
    if (!pwd)
        return (false);
    ft_envaddback(&data->env, pwd);
    
    t_env *shlvl = ft_envnew(ft_strdup("SHLVL"), ft_strdup("1"));
    if (!shlvl)
        return (false);
    ft_envaddback(&data->env, shlvl);
    
    t_env *underscore = ft_envnew(ft_strdup("_"), ft_strdup("/usr/bin/env"));
    if (!underscore)
        return (false);
