...
static bool validate_and_scan_quotes(t_lexer *lexer, char quote_type, int before_quote, int *inside_start_pos, int *len)
{
    advance_lexer(lexer);
...
t_token *read_quotes(t_lexer *lexer)
{
    ...
    if (!validate_and_scan_quotes(lexer, quote_type, before_quote, &inside_start_pos, &len))
        return (NULL);
    token = create_token(get_quote_type(quote_type), ft_substr(lexer->input, inside_start_pos, len));
    if (!token)
        return (NULL);
