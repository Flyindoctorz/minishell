...
/*   Updated: 2025/04/02 19:06:10 by lmokhtar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_token	*create_dollar_token(int start_pos)
{
	...
	if (!token)
	{
		handle_error(MNSHL_ERR_MEMORY, "create_dollar_token : token creation failed");
		return (NULL);
	}
...
t_token	*create_expand_token(t_lexer *lexer, int start_pos)
{
	...
	if (!var_name)
	{
		handle_error(MNSHL_ERR_MEMORY, "create_expand_token : ft_substr failed");
		return (NULL);
	}
	...
	if (!token)
	{
		handle_error(MNSHL_ERR_MEMORY, "create_expand_token : token creation failed");
		return (NULL);
	}
	token->position = start_pos;
